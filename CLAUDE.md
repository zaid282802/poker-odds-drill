...
Eight drills: `outs`, `potodds`, `combos`, `equity`, `callfold`, `polar`, `bluff`, `market`.
...
3. **Size cap 80 KB** for `index.html`. Currently about 61 KB, so 76% used. Raised from 60 KB when
   phase 2 landed; still a single file with zero dependencies.
...
- **`bucket()` returns an archetype, not a card instance.** Adaptive selection deals two to four
  candidate spots and keeps the one whose bucket you are worst at. Buckets with fewer than three
  reps count as unseen and win outright, so new archetypes surface early. `CANDIDATES` sets the
  candidate count per drill, lower for drills whose deal costs a Monte Carlo run.
- **In `polar`, over-bluffing always implies calling.** Not a coincidence to be "fixed": his bluff
  share exceeding `b/(P+2b)` and your equity exceeding `b/(P+2b)` are the same inequality from the
  two seats. There is a test asserting they never disagree.
...
Done in v0.2.0: adaptive bucketing, explicit range notation, and the `polar` value/bluff drill.

1. **Action-to-range inference across streets.** `polar` gives villain a named range on the river,
   but nothing yet asks "he bet flop, checked turn, overbet river, so what is left". That needs an
   action model, which is a design problem, not just code.
2. **Multiway in `callfold` and `market`.** Only `equity` supports 2-3 opponents today.
3. `position`: hand plus 6-max seat, graded against a standard opening chart.
4. Light theme.
5. Tournament and ICM math. Different book, different drill set.
